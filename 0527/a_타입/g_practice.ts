{
  ...
  function doubleOrNothing(double: Union) {
  ...
  let numDouble = doubleOrNothing(24);
  let strDouble = doubleOrNothing('24');
  ...
  const user:Person = {id:2, username:'pyj'};
  identifyPerson(adminPerson)
  identifyPerson(user)

  //! 4. 실제 프로젝트에서의 union 사용
  ...
  function handleResponse(response: UserResponse) {
    // in 연산자 (속성명 in 객체명)
    // : 객체의 속성이 존재하면 true, 없을 경우 false 반환
  ...
  handleResponse({ name:'박영준', email: 'qwe123'});
  ...
  console.log(newCar); // { make: 'kia', model: 'k5', engineType: '가솔린', horsepower: 100 }
  ...
  console.log(assignProjectToTeamMember(memb, proj));
}
