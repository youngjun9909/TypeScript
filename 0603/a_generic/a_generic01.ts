// a_generic01.ts
{
  ...
  // let stringOutput = generic('안녕하세요'); - 오류나지않음
  // let stringOutput2 = generic<string>(500); - Error

  let numberOutput = generic<number>(500);
  // let numberOutput = generic(500);  - 오류나지않음
  // let numberOutput = generic<number>('반갑습니다'); - Error
  ...
  let pairOutput = pair('안녕', 123);
  let pairOutput2 = pair<string, number>('안녕', 123);
  ...
  let resultFunc = genericFunc<boolean>([false, true, false]);
  ...
  console.log(myGenericString.add('박', '영준'));
